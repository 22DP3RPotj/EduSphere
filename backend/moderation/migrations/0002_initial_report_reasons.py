...
def create_reasons(apps, schema_editor):
    ...
    user_ct = ContentType.objects.get(app_label="account", model="user")
    room_ct = ContentType.objects.get(app_label="room", model="room")

    for label in REASONS_ALL:
        ReportReason.objects.update_or_create(label=label, defaults={"is_active": True})

    for label in REASONS_USER_ONLY:
        reason, _ = ReportReason.objects.update_or_create(label=label, defaults={"is_active": True})
